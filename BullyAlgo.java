...
//        new Thread(sender).start();
    ...
    public static void sendCoordinatorMsg() throws IOException {
        for (int key : processes.keySet()) {
            if (key != self_id) {
                ...
                try {
                    System.out.println("DESTINATION SERVER: " + destination_server);
                    Socket socket = new Socket(destination_server, server_Port);
                    DataOutputStream out = new DataOutputStream(socket.getOutputStream());
                    out.writeUTF("cood");
    ...
    public static void initialize() {
        processes.put(2, "192.168.199.120"); // receiver
        processes.put(1, "192.168.199.167"); // sender
